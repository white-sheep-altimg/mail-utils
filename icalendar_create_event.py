...
def create_event(store, title, start, end, cal, notes):
  event = store.add_event(title=title, start=start, end=end, calendar=cal, notes=notes)
  return event
...
if __name__ == "__main__":
  ...
  if len(sys.argv) >= 6:
    ...
    print(f"       python {sys.argv[0]} <タイトル> <開始日時> <終了日時> <カレンダー> <note>", file=sys.stderr)
  ...
  event = create_event(store, title, start, end, cal, notes)
